...
def EscuchaDadoPorUsuarios():
    ...
    mn=maineval2.Main()
    mn.LoadDataEval()
    rts_gp = mn.UserRating.groupby(by=['song_id']).agg({'listen_count': 'count'}).reset_index()
    rts_gp.columns = ['SongId', 'Count']
    plt.barh(rts_gp.SongId, rts_gp.Count, color='royalblue')
    plt.title('Overall Count of ListenCount by Song', fontsize=15)
...
def GraphPerformances():
    ...
    plt.subplot(1, 2, 1)
    plt.title('Comparación de Algoritmo en RMSE', loc='left', fontsize=15)
    plt.plot(lstrmsealgorithm1, rmse1, label='RMSE', color='darkgreen', marker='o')
    plt.xlabel('Metodo Minima Satisfaccion', fontsize=15)
    plt.ylabel('RMSE', fontsize=15)
    plt.legend()
    plt.grid(ls='dashed')    
        
    plt.subplot(1, 2, 2)
    plt.title('Comparicion de Algorithmo en MAE ', loc='center', fontsize=15)
    ...
    plt1.subplot(1, 2, 1)
    plt1.title('Comparacion de Algoritmo en RMSE', loc='left', fontsize=15)
    plt1.plot(lstrmsealgorithm2, rmse2, label='RMSE', color='red', marker='o')
    plt1.xlabel('Metodo Promedio ', fontsize=15)
    plt1.ylabel('RMSE', fontsize=15)
    plt1.legend()
    plt1.grid(ls='dashed')    
        
    plt1.subplot(1, 2, 2)
    plt1.title('Comparacion de Algoritmo en MAE', loc='center', fontsize=15)
    ...
    plt2.subplot(1, 2, 1)
    plt2.title('Comparacion de Algoritmo en RMSE', loc='left', fontsize=15)
    plt2.plot(lstrmsealgorithm3, rmse3, label='RMSE', color='yellow', marker='o')
    plt2.xlabel('Metodo Maxima Satisfaccion', fontsize=15)
    plt2.ylabel('RMSE', fontsize=15)
    plt2.legend()
    plt2.grid(ls='dashed')

    plt2.subplot(1, 2, 2)
    plt2.title('Comparacion de Algoritmo en MAE', loc='center', fontsize=15)
